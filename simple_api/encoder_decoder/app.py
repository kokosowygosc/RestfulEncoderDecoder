    ...
    def encode(self, text_to_encode):
        ...
        for word in words:
            if len(word) > 3 and len(set(word[1:-1])) != 1:
                shuffled_sentence += word[0] + self.shuffler(word[1:-1]) + word[-1]
...
class Decoder:
    ...
    def decode(self, text_to_decode):
        ...
        encoded_text, original_words = filter(None, text_to_decode.split('\n---weird---\n'))
        ...
        for word in encoded_text:
            ...
            if len(word) > 3 and len(set(word[1:-1])) != 1 and re.match(r'(\w+)', word):
